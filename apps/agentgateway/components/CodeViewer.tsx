    ...
    <div className={containerClassName}>
      <div className="grid min-w-0 gap-2 border-b px-3 py-2 sm:flex sm:items-center sm:justify-between">
        ...
        <div className="grid min-w-0 grid-cols-3 gap-2 sm:flex sm:justify-end">
          <Button type="button" variant="outline" size="sm" onClick={() => setExpanded((value) => !value)} aria-label={expanded ? "Collapse editor" : "Expand editor"} className="min-w-0 px-2 sm:px-3" disabled={!hasCode}>
            {expanded ? <Minimize2 className="h-4 w-4" /> : <Maximize2 className="h-4 w-4" />}
            <span className="truncate">{expanded ? "Collapse" : "Expand"}</span>
          </Button>
          <Button type="button" variant="outline" size="sm" onClick={copyCode} className="min-w-0 px-2 sm:px-3" disabled={!hasCode}>
            {copied ? <Check className="h-4 w-4" /> : <Copy className="h-4 w-4" />}
            <span className="truncate">{copied ? "Copied" : "Copy"}</span>
          </Button>
          <Button type="button" size="sm" onClick={downloadCode} aria-label={`Download ${fileName}`} className="min-w-0 px-2 sm:px-3" disabled={!hasCode}>
