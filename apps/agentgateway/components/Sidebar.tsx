...
const mcpLinks = [
  ...
  { href: "/dashboard/generate", label: "Generate endpoint", mobileLabel: "Generate", icon: Wand2 },
  { href: "/dashboard/servers", label: "Servers", icon: Code2 },
  ...
  { href: "/dashboard/api-keys", label: "API keys", mobileLabel: "API keys", icon: KeyRound },
];
...
const mcpMobileLinkGroups = [
  { label: "Build", items: mcpLinks.slice(0, 6) },
  { label: "Operate", items: mcpLinks.slice(6) },
];
...
function ProductSwitcher({ pathname }: { pathname: string }) {
  ...
  return (
    <div className="relative w-full">
      ...
      >
        <Image src="/brand/astrail-prism-icon-inverse.svg" alt="" width={512} height={512} className="h-9 w-9 shrink-0" />
...
export function Sidebar() {
  ...
  const activeLinks = isSdkMode ? sdkLinks : mcpLinks;
      ...
      <aside className="hidden overflow-hidden border-b border-white/10 bg-[#111113] text-white md:sticky md:top-0 md:block md:h-screen md:min-h-0 md:w-[260px] md:shrink-0 md:self-start md:border-b-0 md:border-r md:border-r-white/10">
        ...
        <div className="flex flex-col md:h-[calc(100vh-4rem)]">
          <div className="min-w-0 p-3 md:min-h-0 md:flex-1 md:overflow-y-auto">
            <nav className="md:block md:space-y-1">
              {activeLinks.map((item) => {
                ...
                return (
                  <Link
                    key={item.href}
                    href={item.href}
                    className={`flex min-w-0 items-center gap-3 rounded-lg px-3 py-2.5 text-sm transition-colors hover:bg-white/[0.07] hover:text-white ${
                      active ? "bg-white/[0.09] text-white shadow-sm" : "text-white/52"
                    }`}
                  >
