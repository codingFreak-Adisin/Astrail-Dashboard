...
export function CodeViewer({ code, fileName = "server.ts" }: { code: string; fileName?: string }) {
  ...
  const containerClassName = expanded
    ? "fixed inset-3 z-[90] min-w-0 overflow-hidden rounded-2xl border border-neutral-200/70 bg-white shadow-lg sm:inset-6"
    : "console-card min-w-0 self-start overflow-hidden xl:sticky xl:top-4";

  return (
    <div className={containerClassName}>
      <div className="grid min-w-0 gap-2 border-b border-neutral-100 px-3 py-2 sm:flex sm:items-center sm:justify-between">
        <span className="min-w-0 truncate font-mono text-sm text-neutral-500">{fileName}</span>
        {hasCode ? (
          ...
          <span className="pill pill-neutral w-fit">
      ...
      {hasCode ? (
        ...
        <div className="grid min-h-[220px] place-items-center p-6">
          <div className="max-w-sm text-center">
            <div className="mx-auto grid h-10 w-10 place-items-center rounded-xl border border-neutral-200/70 bg-neutral-50">
              <FileCode2 className="h-5 w-5 text-neutral-400" />
            </div>
            <p className="mt-3 font-medium text-neutral-950">No source saved</p>
