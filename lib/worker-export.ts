...
const server = http.createServer(async (request, response) => {
  ...
  try {
    ...
    writeJson(response, 502, { error: error instanceof Error ? error.message : "Astrail Docker MCP export failed." });
  }
});
...
The Docker image runs as the non-root \`node\` user and exposes a local \`/health\` probe. Pass secrets at runtime through your deployment platform; do not bake \`ASTRAIL_API_KEY\` into the image.
