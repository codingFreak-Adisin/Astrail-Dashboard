...
import { requireTurnstileChallenge } from "@/lib/turnstile";
...
export async function POST(request: Request) {
  if (!hasServerSupabaseEnv()) {
    ...
    try {
      ...
      bodyForFailure = body;
      const challenge = await requireTurnstileChallenge(request, body.turnstileToken, "generate-mcp");
      if (!challenge.ok) {
        return NextResponse.json({ error: challenge.error }, { status: challenge.status ?? 403 });
      }
      const { turnstileToken: _turnstileToken, ...generationBody } = body;
      const pipeline = await runGenerationPipeline(generationBody);
      const selfDescribingId = openApiPreviewServerId(generationBody);
      const localPreview = buildLocalOpenApiPreviewServer(generationBody, pipeline, request.url, selfDescribingId);
      if (!selfDescribingId) {
  ...
  try {
    ...
    const { turnstileToken: _turnstileToken, ...generationBody } = body;
    const generationAllowance = await checkGenerationAllowance(userData.user.id, generationBody.sourceType);
    ...
    const baseInsert = {
      ...
      source_type: generationBody.sourceType,
