              ...
              <div className="mt-1 border-t border-neutral-100 p-3">
                ...
                <div className="mt-3 grid gap-3 sm:grid-cols-2">
                  <label className="grid min-w-0 gap-1 text-xs font-medium text-neutral-500">
                    ...
                    <input
                      ...
                      className="h-10 min-w-0 rounded-xl border border-neutral-200 bg-white px-3 font-mono text-sm text-neutral-950 outline-none transition focus:border-orange-400 focus:ring-2 focus:ring-orange-100"
                    />
                  </label>
                  <label className="grid min-w-0 gap-1 text-xs font-medium text-neutral-500">
                    To
                ...
                <div className="mt-3 flex items-center justify-between gap-3">
                  ...
                  <button
                    ...
                    className="rounded-full bg-neutral-950 px-3.5 py-2 text-xs font-medium text-white transition hover:bg-neutral-800 disabled:cursor-not-allowed disabled:bg-neutral-200 disabled:text-neutral-500"
                  >
                    Apply
                  </button>
                </div>
      ...
      <section className="section-card">
        <div className="section-card-header">
          <div>
            ...
            <p className="mt-0.5 text-xs text-neutral-400">{formatRange(buckets)}</p>
          </div>
      ...
      <section className="section-card">
        <div className="section-card-header">
          <div>
            ...
            <p className="mt-0.5 text-xs text-neutral-400">0 premium calls</p>
          </div>
        </div>
  ...
  return (
    <section className="grid gap-4 lg:grid-cols-[minmax(0,1fr)_340px]">
      <div className="console-card overflow-hidden p-0">
        <div className="flex flex-wrap items-start justify-between gap-4 border-b border-neutral-100 px-5 py-5">
          <div>
            <p className="text-xs text-neutral-400">Range credit spend</p>
            <div className="mt-2 flex flex-wrap items-end gap-3">
              <h2 className="font-mono text-4xl font-semibold tabular-nums tracking-tight text-neutral-950">{rangeCredits.toLocaleString()}</h2>
              <span className="pb-1 text-sm text-neutral-500">credits from accepted tool calls</span>
            </div>
          </div>
          <div className="flex items-center gap-2">
            <span className="pill pill-neutral">Group by 1d</span>
            <span className="pill pill-neutral">{activeRange.label}</span>
          </div>
        </div>
      ...
      <aside className="console-card p-5">
        <div className="flex items-start justify-between gap-4">
          <div>
            <p className="text-xs text-neutral-400">Monthly credit cap</p>
            <h2 className="mt-2 text-2xl font-semibold text-neutral-950">{usage.planName}</h2>
          </div>
          <span className="pill pill-brand">{formatBillingStatus(usage.status)}</span>
        </div>

        <div className="mt-5">
          <div className="flex items-center justify-between gap-3 text-sm">
            <span className="text-neutral-500">Used this period</span>
            <span className={`font-mono ${creditOverage > 0 ? "text-red-600" : "text-neutral-950"}`}>{creditUsageLabel}</span>
          </div>
          <div className="mt-3 h-1.5 w-full overflow-hidden rounded-full bg-neutral-100">
            <div className={`h-full rounded-full ${creditOverage > 0 ? "bg-red-500" : "bg-orange-400"}`} style={{ width: `${budgetWidth}%` }} />
          </div>
        ...
        <div className="mt-6 grid gap-3 border-t border-neutral-100 pt-5">
          ...
          return (
            <div key={buckets[index].key} className="group relative flex min-w-0 flex-1 items-end">
              <div
                ...
                style={{ height: `${height}%` }}
              />
              <div className="pointer-events-none absolute bottom-full left-1/2 z-10 mb-2 hidden -translate-x-1/2 whitespace-nowrap rounded-xl border border-neutral-200 bg-white px-2 py-1 text-xs shadow-lg group-hover:block">
...
function UsageBreakdownRow({ label, value, sub }: { label: string; value: string; sub: string }) {
  return (
    <div className="flex items-center justify-between gap-4 rounded-xl bg-neutral-50/50 px-3 py-3">
      <div>
        <div className="text-sm text-neutral-500">{label}</div>
        <div className="mt-1 text-xs text-neutral-400">{sub}</div>
      </div>
      <div className="font-mono text-lg font-semibold tabular-nums text-neutral-950">{value}</div>
    </div>
  );
}
  ...
  return (
    <div className="relative">
      <button
        type="button"
        onClick={onToggle}
        className="inline-flex items-center gap-2 rounded-full border border-neutral-200 bg-white px-4 py-2 text-sm shadow-sm transition hover:border-neutral-300 hover:bg-neutral-50"
        aria-expanded={open}
      >
      ...
      {open ? (
        <div className={`absolute left-0 z-20 mt-2 rounded-xl border border-neutral-200 bg-white shadow-sm ${menuClassName}`}>
          {children}
        </div>
      ) : null}
    </div>
...
function MetricCard({
  ...
  return (
    ...
    >
      ...
      <div className="mt-4 font-mono text-3xl font-semibold tabular-nums tracking-tight text-neutral-950">{value}</div>
    </button>
  );
}
...
function UsageChart({ buckets, metrics }: { buckets: UsageBucket[]; metrics: MetricKey[] }) {
  ...
  return (
    <div className="relative mt-6 overflow-hidden">
      ...
      {activeBucket ? (
        <div
          className="pointer-events-none absolute top-4 min-w-44 rounded-xl border border-neutral-200 bg-white p-3 text-xs shadow-sm"
...
function BarRow({ label, value, max }: { label: string; value: number; max: number }) {
  ...
  return (
    <div className="grid grid-cols-[minmax(0,160px)_1fr_40px] items-center gap-4 text-sm">
      ...
      <span className="text-right font-mono tabular-nums text-neutral-700">{value}</span>
    </div>
  );
}
    ...
    <div>
      <div className="flex flex-wrap items-start justify-between gap-2 text-sm">
        ...
        <span className="flex flex-wrap items-center justify-end gap-2 text-right">
          <span className={`font-mono text-xs tabular-nums ${overLimit > 0 ? "text-red-600" : "text-neutral-700"}`}>{displayUsage}</span>
          {overLimit > 0 ? (
            <span className="pill pill-danger">Upgrade needed</span>
          ) : null}
        </span>
      </div>
