...
export type CodeModeCredentialLoader = (server: McpServer, tool: McpTool) => Promise<RuntimeCredential | null>;
...
export const staticNoEvalSdkAdapter: CodeModeSandboxAdapter = {
  ...
  async execute(server, args, options) {
    ...
    for (const call of plan.calls) {
      ...
      const credential = await options.loadCredentialForTool(server, endpointTool);
