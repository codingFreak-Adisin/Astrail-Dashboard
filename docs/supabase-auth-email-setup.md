...
For non-public Preview or Development environments, `RATE_LIMIT_MODE=in_memory` is acceptable only when the deployment is access-protected and not used for public MCP traffic. Production should use Redis-backed limits plus the edge-protection attestations after completing `docs/EDGE_DDOS_WAF_SETUP.md`.
