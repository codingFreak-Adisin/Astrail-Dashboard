...
import { loadRuntimeCredentialForTool } from "@/lib/runtime/credential-loader";
...
async function handleJsonRpcRequest(bundle: Bundle, servers: McpServer[], body: unknown, approvedExecutionId?: string): Promise<JsonRpcHandlerResult> {
  if (!isJsonRpcRequest(body) || hasInvalidJsonRpcId(body)) {
  ...
  if (body.method === "tools/call") {
    ...
    const execution = server.source_type === "preset" && (!Array.isArray(server.endpoint_map) || server.endpoint_map.length === 0)
      ...
      : await executeToolFromEndpointMap(server, tool, args, { credential: await loadRuntimeCredentialForTool(server, tool) });
