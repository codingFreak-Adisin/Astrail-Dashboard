...
import { auditMcpSecurityEvent, humanToolCallSummary, sanitizeToolLogRecord, writeStructuredLog } from "@/lib/runtime/observability";
...
async function validateOwnerApiKey(server: McpServer, rawKey: string | null) {
  if (server.is_public && !rawKey) return { endUserId: null, actorRole: null, allowHeaderContext: false } satisfies ApiKeyAuthorization;
  ...
  if (server.user_id === localDemoUserId && process.env.ASTRAIL_ENABLE_LOCAL_SECURITY_FIXTURES === "1") {
    ...
    return rawKey === (process.env.ASTRAIL_LOCAL_MCP_API_KEY ?? "ag_demo_secret")
      ? { endUserId, actorRole, allowHeaderContext: !endUserId && !actorRole } satisfies ApiKeyAuthorization
      : null;
  }
...
function callerContext(request: Request, authorization: ApiKeyAuthorization) {
...
const MAX_LOGGED_ARGUMENT_BYTES = 4_000;

function redactedArgumentsForLog(args: Record<string, unknown> | null | undefined) {
  if (!args || Object.keys(args).length === 0) return null;
  const redacted = sanitizeToolLogRecord(args);
  try {
    if (JSON.stringify(redacted).length > MAX_LOGGED_ARGUMENT_BYTES) {
      return { truncated: true, argument_names: Object.keys(args).slice(0, 50) };
    }
  } catch {
    return null;
  }
...
async function logToolExecution(server: McpServer, toolName: string, execution: ToolExecutionResult, context: ToolLogContext = {}) {
  ...
  try {
    ...
    if (error?.message.includes("column")) {
      // Deployment has not run the integration-hardening migration yet; fall back to the legacy column set.
      const legacy = await admin.from("tool_call_logs").insert(baseRecord);
      if (legacy.error) {
        writeStructuredLog({ ...logPayload, storage: "structured_log", storage_error: legacy.error.message });
      }
    } else if (error) {
...
async function loadCredentialResultForTool(server: McpServer, tool: McpTool, endUserId: string | null = null) {
  return loadRuntimeCredentialResultForTool(server, tool, { endUserId });
}
...
async function executeMetaTool(server: McpServer, tool: McpTool, args: Record<string, unknown>, endUserId: string | null = null) {
  ...
  if (tool.name === "execute") {
    return executeSdkCodeMode(server, args, { loadCredentialForTool: (codeModeServer: McpServer, codeModeTool: McpTool) => loadCredentialResultForTool(codeModeServer, codeModeTool, endUserId) });
  }
  ...
  if (tool.name === "invoke_api_endpoint") {
    ...
    const credentialResult = await loadCredentialResultForTool(server, endpointTool, endUserId);
...
export async function GET(request: Request, { params }: { params: { serverId: string } }) {
  ...
  const authorization = await validateOwnerApiKey(server, getBearerToken(request));
  if (!authorization) {
...
async function handleJsonRpcRequest(server: McpServer, body: unknown, approvedExecutionId?: string, endUserId: string | null = null, actorRole: string | null = null): Promise<JsonRpcHandlerResult> {
  if (!isJsonRpcRequest(body) || hasInvalidJsonRpcId(body)) {
  ...
  if (body.method === "tools/call") {
    ...
    const logContext: ToolLogContext = { endUserId, actorRole };
    if (denied) {
    ...
    if (isMetaTool) {
      execution = await executeMetaTool(server, tool, toolArgs, endUserId);
...
async function handleBatchItem(server: McpServer, item: unknown, endUserId: string | null, actorRole: string | null): Promise<JsonRpcHandlerResult> {
  return isolateBatchItem(
    () => handleJsonRpcRequest(server, item, undefined, endUserId, actorRole),
    () => {
      const traceId = createRuntimeTraceId();
      writeStructuredLog({
...
export async function POST(request: Request, { params }: { params: { serverId: string } }) {
  ...
  const context = callerContext(request, authorization);
  if (context.error) {
  ...
  if (Array.isArray(body)) {
    const results = await Promise.all(body.map((item) => handleBatchItem(server, item, endUserId, actorRole)));
  ...
  const result = await handleJsonRpcRequest(server, body, undefined, endUserId, actorRole);
  if (!result.payload) {
